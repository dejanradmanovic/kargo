...
fn test_load_env_from_fixture() {
    ...
    assert_eq!(env_vars.get("SENTRY_DSN").unwrap(), "https://def@sentry.io/456");
    assert_eq!(env_vars.get("DATABASE_URL").unwrap(), "postgres://localhost/testdb");
...
fn test_interpolate_with_fixture_env() {
    ...
    assert_eq!(result, "key=AIzaSyBTestKey456 dsn=https://def@sentry.io/456");
}
...
fn test_interpolate_mixed_env_refs() {
    ...
    let result = interpolate(input, &env_overrides);
    assert_eq!(result, "db=postgres://localhost/testdb token=test-token-abc123");
}
