//! JavaScript Kotlin compiler (kotlinc-js) invocation.
//!
...
impl TargetCompiler for JsCompiler {
    fn compile(
        &self,
        ...
        Err(kargo_util::errors::KargoError::Generic {
            message: format!(
                "Compilation for target {} is not yet supported. JVM builds are available.\n  \
                 JS target support is planned for Phase 5 (KMP).",
                self.target
            ),
        }
        .into())
    ...
    fn compiler_binary(&self, toolchain: &ToolchainPaths) -> PathBuf {
        toolchain.home.join("bin").join("kotlinc-js")
    }
}
